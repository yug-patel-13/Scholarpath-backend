...
export class InitialSchema1700000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Foreign Keys - Only create if they don't exist
    const scholarshipsTable = await queryRunner.getTable('scholarships');
    const scholarshipsFkExists = scholarshipsTable?.foreignKeys.some(
    ...
    const userProfilesFkExists = userProfilesTable?.foreignKeys.some(
    ...
    const formFillRequestsFkExists = formFillRequestsTable?.foreignKeys.some(
      (fk) => fk.columnNames.includes('userId'),
    );
    ...
    const usersIndexExists = usersTable?.indices.some(
      (idx) => idx.name === 'IDX_USER_EMAIL',
    );
    if (!usersIndexExists) {
    ...
    const cafesIndexExists = cyberCafesTable?.indices.some(
      (idx) => idx.name === 'IDX_CAFE_LOCATION',
    );
    if (!cafesIndexExists) {
      await queryRunner.createIndex(
        'cyber_cafes',
